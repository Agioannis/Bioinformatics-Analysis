...
def sequence_alignment(seq1, seq2, method="nw"):
    ...
    # Ορισμός συστήματος βαθμολόγησης
    aligner.match_score = 2
    aligner.mismatch_score = -1
    aligner.open_gap_score = -5
    aligner.extend_gap_score = -0.5

    if method == "nw":  # Needleman-Wunsch (παγκόσμια στοίχιση)
        aligner.mode = "global"
    elif method == "sw":  # Smith-Waterman (τοπική στοίχιση)
        aligner.mode = "local"
    else:
        raise ValueError("Μη έγκυρη μέθοδος. Χρησιμοποίησε 'nw' ή 'sw'.")

    alignments = aligner.align(seq1, seq2)
    if not alignments:
        return None

    aln = alignments[0]  # καλύτερη στοίχιση
    aligned_seq1 = aln.aligned[0]
    aligned_seq2 = aln.aligned[1]

    # Μετατροπή της στοίχισης σε αναγνώσιμη μορφή
    alignment_str = str(aln)

    # Υπολογισμός ταυτότητας (προσεγγιστικά: σύγκριση των στοίχισμένων βάσεων)
    aligned_str1, aligned_str2 = alignment_str.split("\n")[:2]
    matches = sum(a == b for a, b in zip(aligned_str1, aligned_str2) if a != "-" and b != "-")
    identity = (matches / min(len(seq1), len(seq2))) * 100

    # Καταμέτρηση κενών (gaps)
    gaps = aligned_str1.count("-") + aligned_str2.count("-")
