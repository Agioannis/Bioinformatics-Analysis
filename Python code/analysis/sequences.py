    ...
    for motif in motif_patterns:
        ...
        if positions:
            motif_results[motif] = {
                'positions': positions,
                'count': len(positions),
                'density': len(positions) / len(s) * 1000
            }
...
def comprehensive_analysis(seq: Seq, seq_id: str) -> Dict:
    ...
    # For molecular weight, remove N bases as they're ambiguous
    ...
    # For simplicity, we run full analyses here; caller may decide to skip.
    try:
        results.update({
